...
// Linear elasticity
  ...
  if (inputData.SbmGeo == LEInputData::SBMGeo::RING and inputData.InsideSBM)
  {
    const double coordsCenter[DIM]{1.0, 1.0};
    subDomain.addObject(VOXEL::Circle(coordsCenter, 1.0 - 1.7 / pow(2, inputData.mesh_def.refine_lvl_base), RetainSide::IN));
    subDomain.addObject(VOXEL::Circle(coordsCenter, 0.25 + 1.7 / pow(2, inputData.mesh_def.refine_lvl_base), RetainSide::OUT));
  }
  else
  {
